import Link from "next/link";
import { getPlants } from "../lib/api";
import type { PlantTaxon } from "../types/plant";

export default async function PlantsPage() {
  let plants: PlantTaxon[] = [];
  let error: string | null = null;

  try {
    plants = await getPlants();
  } catch (e) {
    error = e instanceof Error ? e.message : "Failed to load plants";
  }

  return (
    <div className="flex flex-col gap-8">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-chlorophyll-emerald-800">Plant Taxonomy</h1>
          <p className="mt-2 text-sm text-chlorophyll-emerald-600">
            Manage your plant catalog and botanical information
          </p>
        </div>
        <Link
          href="/plants/new"
          className="inline-flex items-center justify-center rounded-lg bg-chlorophyll-emerald-400 px-6 py-3 text-sm font-semibold text-white shadow-md transition-all hover:bg-chlorophyll-emerald-500 hover:shadow-lg"
        >
          + New Plant
        </Link>
      </div>

      {/* Error state */}
      {error && (
        <div className="rounded-lg border-2 border-red-200 bg-red-50 p-4 text-sm text-red-800">
          {error}
        </div>
      )}

      {/* Plants list */}
      {!error && (
        <>
          {plants.length === 0 ? (
            <div className="rounded-xl border-2 border-chlorophyll-light-200 bg-white p-12 text-center shadow-sm">
              <p className="text-chlorophyll-emerald-600">No plants in catalog yet.</p>
              <Link
                href="/plants/new"
                className="mt-4 inline-block text-sm font-semibold text-chlorophyll-emerald-400 hover:text-chlorophyll-emerald-500"
              >
                Add your first plant →
              </Link>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {plants.map((plant) => (
                <Link
                  key={plant.id}
                  href={`/plants/${plant.id}`}
                  className="group rounded-xl border-2 border-chlorophyll-light-200 bg-white p-6 shadow-sm transition-all hover:border-chlorophyll-light hover:shadow-md"
                >
                  <h2 className="mb-2 text-lg font-semibold text-chlorophyll-emerald-800 group-hover:text-chlorophyll-emerald-700">
                    {plant.botanical_name}
                  </h2>
                  <div className="space-y-1 text-sm text-chlorophyll-emerald-600">
                    {plant.common_name && (
                      <p className="font-medium">{plant.common_name}</p>
                    )}
                    {plant.cultivar && (
                      <p className="text-xs italic text-chlorophyll-emerald-500">
                        {plant.cultivar}
                      </p>
                    )}
                    <div className="flex flex-wrap gap-2 pt-1">
                      {plant.plant_type && (
                        <span className="rounded-full bg-chlorophyll-light-100 px-2 py-0.5 text-xs text-chlorophyll-emerald-700">
                          {plant.plant_type}
                        </span>
                      )}
                      {plant.is_minnesota_native && (
                        <span className="rounded-full bg-chlorophyll-emerald-100 px-2 py-0.5 text-xs text-chlorophyll-emerald-700">
                          Native
                        </span>
                      )}
                    </div>
                  </div>
                </Link>
              ))}
            </div>
          )}
        </>
      )}
    </div>
  );
}

